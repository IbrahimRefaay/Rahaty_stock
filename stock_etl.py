...
# Adapted for GitHub Actions with environment variables for credentials.
...
ODOO_PASSWORD = os.getenv("ODOO_PASSWORD", "Rs.Data.team")  # Read from environment variable
...
def main():
    ...
    try:
        # Use credentials from environment variable set by GitHub Actions
        ...
        # Use pandas-gbq which automatically finds credentials from environment
        final_df.to_gbq(destination_table=f"{DATASET_ID}.{STOCK_TABLE}", 
...
if __name__ == "__main__":
    logging.info("--- Starting Odoo Stock Snapshot to BigQuery ETL Process ---")
    try:
        ...
        main()
        logging.info("--- Stock ETL Process Completed Successfully ---")
    except Exception as e:
        logging.critical(f"--- Stock ETL Process Failed ---", exc_info=True)